...
source_inputs = tf.placeholder(shape=(None, max_len), dtype=tf.int32, name='source_inputs')
target_inputs = tf.placeholder(shape=(None, max_len), dtype=tf.int32, name='y_inputs')
...
loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tf.reshape(target_inputs, [-1]), logits=y_pred))
...
def test_epoch(dataset):
    ...
    for i in xrange(batch_num):
        ...
        feed_dict = {source_inputs:X_batch, target_inputs:y_batch, lr:1e-5, batch_size:_batch_size, keep_prob:1.0}
...
for epoch in xrange(max_max_epoch):
    ...
    for batch in xrange(tr_batch_num):
        ...
        feed_dict = {source_inputs:X_batch, target_inputs:y_batch, lr:_lr, batch_size:tr_batch_size, keep_prob:0.5}
