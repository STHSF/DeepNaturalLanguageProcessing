...
class char_embedding():
    def __init__(self):
        ...
        self.word_embeddings = word_embedding(self.char_ids, self.char_size, self.char_embedding_size,
...
def word_embedding(char_ids, char_size, char_embedding_size, word_lengths, lstm_hidden_size_char_embedding):
    ...
    with tf.variable_scope("bi_lstm_char_embedding"):
        # bi lstm on chars
        cell_fw = tf.contrib.rnn.LSTMCell(lstm_hidden_size_char_embedding, state_is_tuple=True)
        cell_bw = tf.contrib.rnn.LSTMCell(lstm_hidden_size_char_embedding, state_is_tuple=True)
    ...
    word_embeddings = tf.reshape(output, shape=[s[0], s[1], 2 * lstm_hidden_size_char_embedding])

    return word_embeddings


def add_logits_op(word_embeddings, lstm_hidden_size_sentence_embedding, sequence_lengths, dropout, num_classes):
    """Defines self.logits
    For each word in each sentence of the batch, it corresponds to a vector
    of scores, of dimension equal to the number of tags.
    """
    with tf.variable_scope("bi_lstm_sentence_embedding"):
        cell_fw = tf.contrib.rnn.LSTMCell(lstm_hidden_size_sentence_embedding)
        cell_bw = tf.contrib.rnn.LSTMCell(lstm_hidden_size_sentence_embedding)
    ...
    with tf.variable_scope("output"):
        w = tf.get_variable("W", dtype=tf.float32,
                            shape=[2 * lstm_hidden_size_sentence_embedding, num_classes])

        b = tf.get_variable("b", shape=[num_classes],
                            dtype=tf.float32, initializer=tf.zeros_initializer())

        nsteps = tf.shape(output)[1]
        output = tf.reshape(output, [-1, 2 * lstm_hidden_size_sentence_embedding])
        pred = tf.matmul(output, w) + b
        logits = tf.reshape(pred, [-1, nsteps, num_classes])
