...
with open(data_path + 'data.pkl', 'rb') as pk:
    X = pickle.load(pk)
    y = pickle.load(pk)
...
def acc_crf(unary_scores, y_batch, transition_params):
    ...
    for unary_score_, y_ in zip(unary_scores, y_batch):  # unary_score_  :[num_steps, target_num], y_: [num_steps]
        viterbi_prediction, _ = tf.contrib.crf.viterbi_decode(unary_score_, transition_params)
        # viterbi_decode: tuple (list[id], value)
        # y_: tuple
        correct_labels += np.sum(
            np.equal(viterbi_prediction, y_))  # compare prediction sequence with golden sequence
        total_labels += len(y_)
...
def run_epoch(dataset, session, model):
...
with tf.Session() as session:
    ...
    summary_writer = tf.summary.FileWriter('data/model/tensorflowlogs', session.graph)
    ...
    for epoch in range(max_max_epoch):
        ...
        train_perplexity = run_epoch(data_train, session, train_model)
        print("Epoch: %d Train acc: %.3f, Train cost: %.3f " % (epoch + 1, train_perplexity[0], train_perplexity[1]))
        valid_perplexity = run_epoch(data_valid, session, valid_model)
        print("Epoch: %d Valid acc: %.3f, Valid cost: %.3f " % (epoch + 1, valid_perplexity[0], valid_perplexity[1]))

    test_perplexity = run_epoch(data_test, session, test_model)
